...
class AACPolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, hidden_dims:int, **kw):
        ...
        self.policy = nn.Sequential(nn.Linear(hidden_dims, out_actions))
        self.value = nn.Sequential(nn.Linear(hidden_dims, 1))
    ...
    def forward(self, x):
        ...
        #  given state.
        # ====== YOUR CODE: ======
        action_scores = self.policy(self.fc(x))
        state_values = self.value(self.fc(x))
        # ========================
    ...
    def build_for_env(env: gym.Env, device="cpu", **kw):
        ...
        net = AACPolicyNet(in_features=in_features, out_actions=out_actions, **kw)
        # ========================
...
class AACPolicyAgent(PolicyAgent):
    def current_action_distribution(self) -> torch.Tensor:
        ...
        # ====== YOUR CODE: ======
        with torch.no_grad():
            actions_proba, _ = self.p_net.forward(self.curr_state)
