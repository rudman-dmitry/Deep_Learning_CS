...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        for experience in self.experiences[-2::-1]:
            qvals.insert(0, qvals[0] * gamma + experience.reward)
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        # ====== YOUR CODE: ======
        """
        experiences = [episode.experiences for episode in episodes]
        experiences = [e for experience in experiences for e in experience]
        states = [e.state for e in experiences]
        actions = [e.action for e in experiences]
        rewards = [e.reward for e in experiences]
        q_values = [episode.calc_qvals(gamma) for episode in episodes]
        q_values = [val for qvals in q_values for val in qvals]
        train_batch = torch.transpose(torch.stack([states, actions, rewards, q_values], dim=0), 0, 1)
        """
        total_rewards = [e.total_reward for e in episodes]
        states = [e.state for episode in episodes for e in episode.experiences]
        actions = [e.action for episode in episodes for e in episode.experiences]
        q_values = [episode.calc_qvals(gamma=gamma) for episode in episodes]
        q_values = [val for episode_vals in q_values for val in episode_vals]
        train_batch = TrainBatch(torch.stack(states, dim=0), torch.tensor(actions), \
                                 torch.tensor(q_values), torch.tensor(total_rewards))
        # ========================
        return train_batch
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while not is_done:
                ...
                is_done = curr_experience.is_done
