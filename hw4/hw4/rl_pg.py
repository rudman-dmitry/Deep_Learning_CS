...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, hidden_dims: int, **kw):
        """
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features, hidden_dims, bias=True),
            nn.ReLU(),
            nn.Linear(hidden_dims, out_actions, bias=True)  #,
            #nn.Softmax(dim=1)
        )
        # ========================
    ...
    def build_for_env(env: gym.Env, device="cpu", **kw):
        ...
        net = PolicyNet(in_features=in_features, out_actions=out_actions, **kw)
        # ========================
...
class PolicyAgent(object):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            actions_proba = torch.softmax(actions_proba, dim=0)
        # ========================
    ...
    def step(self) -> Experience:
        ...
        action_chosen = torch.multinomial(actions_proba, 1).item()
        new_state, reward, is_done, extra_info = self.env.step(action_chosen)
        experience = Experience(self.curr_state, action_chosen, reward, is_done)
        self.curr_state = torch.tensor(new_state)
    ...
    def monitor_episode(
        ...
        """
        Runs a single episode with a Monitor using a specified policy network.
        ...
        with gym.wrappers.Monitor(
            ...
            reward = 0
            while not is_done:
                _, _, curr_reward, is_done = agent.step()
                n_steps += 1
                reward += curr_reward
            # ========================
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        #num_trajectories = batch.total_rewards.shape[0]
        log_proba = torch.log_softmax(input=action_scores, dim=1, dtype=torch.float64)

        a = batch.actions.reshape(-1, 1).repeat(1, log_proba.shape[1])

        log_proba_taken = torch.gather(log_proba, dim=1, index=a)[:, 1]

        # ========================
        return - torch.mean(log_proba_taken*policy_weight)
...
class ActionEntropyLoss(nn.Module):
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        loss_e = torch.mean(torch.sum(torch.softmax(action_scores, dim=1)*
                            torch.log_softmax(action_scores, dim=1), dim=1)/self.max_entropy)
        # ========================
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for loss_f in self.loss_functions:
            loss, loss_dict = loss_f(batch, action_scores)
            total_loss += loss
