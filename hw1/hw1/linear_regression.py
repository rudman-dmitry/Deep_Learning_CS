...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        y_pred = None
        # ====== YOUR CODE: ======
        y_pred = np.matmul(self.weights_, X.T)
        # ========================
    ...
    def fit(self, X, y):
        ...
        I[0][0] = 0
        # Multiplications
        reg = n*self.reg_lambda*I
        X_XT = np.matmul(X.T,X)
        XT_y = np.matmul(y.T,X)
        # Inverse 
        inverse = np.linalg.inv(X_XT+reg)
        w_opt = np.matmul(inverse, XT_y)
        # ========================
...
def fit_predict_dataframe(
    ...
    if feature_names:
        df = df[feature_names]
    y_pred = model.fit_predict(df, target)
    # ========================
    return y_pred
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_f = X**5
        X_cbrt = np.cbrt(X)
        roll_1 = np.multiply(np.roll(X, 1, axis=1), X)
        roll_2 = np.multiply(np.roll(X, 2, axis=1), X)
        roll_3 = np.multiply(np.roll(X, 3, axis=1), X)
        roll_5 = np.multiply(np.roll(X, 5, axis=1), X)
        roll_7 = np.multiply(np.roll(X, 7, axis=1), X)
        X_transformed = np.hstack((X, X_sq, X_cu, neg_exp, neg_exp2, X_sqrt, X_f, X_cbrt, roll_1, roll_2, roll_3, roll_5, roll_7))
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    corr_mat = df.corr(method='pearson')[target_feature]
    # Dropping
    corr_mat = corr_mat.drop(target_feature)
    # Top features
    top_n_corr = corr_mat.abs().sort_values(ascending=False)[:n]
    top_n_features = list(top_n_corr.index)
    top_n_corr = top_n_corr.values
    # ========================
...
def cv_best_hyperparams(
    ...
    # ====== YOUR CODE: ======
    hyper_p = {'linearregressor__reg_lambda': lambda_range,
               'bostonfeaturestransformer__degree': degree_range}
    search = sklearn.model_selection.GridSearchCV(model, param_grid=hyper_p, cv=k_folds)
    search.fit(X, y)
